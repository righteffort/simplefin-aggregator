...
class _RedactPathFilter(logging.Filter):
    def __init__(self, path_prefix: str, replacement: str) -> None:
        super().__init__()
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        if isinstance(args, tuple) and len(args) >= _MIN_ACCESS_LOG_ARGS:
            path = args[2]
            matches = isinstance(path, str) and path.startswith(self.path_prefix)
            if matches:
                record.args = (args[0], args[1], self.replacement, *args[3:])
        return True


def install_access_log_redaction(*, logger_name: str, path_prefix: str, replacement: str) -> None:
    """Redact `replacement` in place of any `path_prefix`-starting path.

    Idempotent per (logger_name, path_prefix): calling this again with the
    same two doesn't add a second filter.
    """
    logger = logging.getLogger(logger_name)
    already_installed = any(
        isinstance(existing, _RedactPathFilter) and existing.path_prefix == path_prefix
        for existing in logger.filters
    )
    if not already_installed:
        logger.addFilter(_RedactPathFilter(path_prefix, replacement))
